...
const AdminDashboard = ({ navigate }) => {
  ...
  const handlePrintTags = async (packageId, tagType) => {
    try {
      ...
      const passengerCount = data.total_passengers || data.total_customers || data.tags.length;
      ...
      const tagsHtml = data.tags.map(t => isId ? `
        <!-- FRONT -->
        <div style="width:220px;background:#fff;border-radius:12px;box-shadow:0 3px 12px rgba(0,0,0,.2);overflow:hidden;page-break-inside:avoid;display:inline-block;margin:8px;vertical-align:top;border:1px solid #ddd">
          <div style="padding:10px;text-align:center;border-bottom:2px solid #c8a84b">
            <div style="display:flex;align-items:center;justify-content:center;gap:8px;margin-bottom:4px">
              ...
              <div style="text-align:left">
                ...
                <div style="font-size:8px;color:#1b5e20;direction:rtl">&#x634;&#x631;&#x643;&#x629; &#x62A;.&#x645; &#x641;&#x648;&#x632;&#x64A; &#x644;&#x644;&#x633;&#x641;&#x631; &#x648;&#x627;&#x644;&#x633;&#x64A;&#x627;&#x62D;&#x629;</div>
              </div>
            </div>
            <div style="display:flex;align-items:center;justify-content:center;gap:5px;margin-top:4px">
              <span>&#x1F1F8;&#x1F1EC;</span>
              <span style="font-size:10px;direction:rtl;color:#333">&#x633;&#x646;&#x63A;&#x627;&#x641;&#x648;&#x631;&#x629;</span>
            </div>
          </div>
          <div style="display:flex;justify-content:center;margin:10px 0 6px">
            <div style="width:75px;height:75px;border-radius:50%;border:3px solid #c8a84b;background:#f5f5f5;display:flex;align-items:center;justify-content:center;font-size:30px">&#x1F464;</div>
          </div>
        ...
        <!-- BACK -->
        <div style="width:220px;background:#f5c518;border-radius:12px;box-shadow:0 3px 12px rgba(0,0,0,.2);overflow:hidden;page-break-inside:avoid;display:inline-block;margin:8px;vertical-align:top">
          <div style="padding:14px 12px">
            <div style="font-size:13px;font-weight:900;color:#111;text-align:center;margin-bottom:10px;letter-spacing:1px">${data.package_name.toUpperCase()}</div>
            <div style="font-size:10px;color:#111;line-height:1.9">
              <div><span style="font-weight:900;text-decoration:underline">MADINAH:</span> ${data.hotel_madinah || 'Hotel TBA'}</div>
              <div style="margin-bottom:4px;color:#333">${data.tel_madinah || '+966 XXXX XXXX'}</div>
              <div><span style="font-weight:900;text-decoration:underline">MAKKAH:</span> ${data.hotel_makkah || 'Hotel TBA'}</div>
              <div style="margin-bottom:8px;color:#333">${data.tel_makkah || '+966 XXXX XXXX'}</div>
            </div>
            <div style="border:1.5px solid #333;border-radius:6px;padding:8px;text-align:center;font-size:10px;color:#111;line-height:1.8">
              <div style="font-weight:900">Tour Leader</div>
              <div>${data.tour_leader_name || 'Contact Tour Leader'}</div>
              <div>${data.tour_leader_phone || '+65 XXXX XXXX'}</div>
            </div>
          </div>
        </div>
      ` : `
            ...
            <div style="flex:1;padding:10px 8px;display:flex;flex-direction:column;justify-content:center">
              <div style="display:flex;align-items:center;gap:8px;margin-bottom:6px">
                ...
                <div>
                  ...
                  <div style="font-size:9px;color:#555;margin-top:2px">Booking: ${t.booking_number}${t.room_number ? ` • Room ${t.room_number}` : ''}</div>
                </div>
              </div>
      ...
      const html = `<!DOCTYPE html><html><head><meta charset="UTF-8"><title>${title} - ${data.package_name}</title>
        ...
        <div class="no-print" style="display:flex;justify-content:space-between;align-items:center;margin-bottom:20px">
          <h2 style="margin:0;color:#1b5e20">${title} – ${data.package_name} (${passengerCount} passengers)</h2>
          <button onclick="window.print()" style="padding:10px 22px;background:#1b5e20;color:#fff;border:none;border-radius:8px;cursor:pointer;font-size:14px;font-weight:bold">🖨️ Print All</button>
        </div>
        <div>${tagsHtml}</div>
      </body></html>`;
      ...
      <div className="lg:ml-64 p-4 md:p-8 pt-20 lg:pt-8">
        <div className="mb-8">
          <h1 className="text-2xl md:text-4xl font-bold text-gray-900 mb-2">
            ...
            {activeSection === 'discount-codes' && '🏷️ Discount Codes'}
            {activeSection === 'messages' && '✉️ Contact Messages'}
                ...
                {modalType.includes('package') && (
                  <>
                    ...
                    <div className="bg-yellow-50 border-2 border-yellow-200 p-4 rounded-lg">
                      <h4 className="font-bold text-gray-900 mb-3">🛏️ Room Prices (per person)</h4>
                      <div className="space-y-3">
                        {[
  ...
  return (
    <div>
      ...
      <div className="bg-white rounded-2xl shadow-lg p-6 mb-6">
        <h3 className="text-lg font-bold mb-4">🎙️ Start New Broadcast</h3>
